...
RANDOM_WHEEL_IMAGE = pygame.image.load("images\\random_wheel.png")
...
BACKGROUND_IMAGE = pygame.image.load("images\sunburst_background.jpg")
...
# def sound_while_rotation(number_before_rotation, number_after_rotation):
    # time_sound = 1 / FPS
    # number_of_rotations = 7
    # total_time = 7.3469 * ( number_after_rotation - number_before_rotation + 49 * number_of_rotations) / (0.5 * 7.3469 * FPS)
    # total_distance = 7.3469 * (number_after_rotation - number_before_rotation + 49 * number_of_rotations)
    # total_distance_durch_73469 = (number_after_rotation - number_before_rotation + 49 * number_of_rotations)
    # a = (7.3469 * ( number_after_rotation - number_before_rotation + 49 * number_of_rotations) - 7.3469 * FPS * total_time) * 2 / (total_time ** 2)
    # v_0 = 7.3469 * 60
    
    # while time_sound < total_time:
    # #     winsound.Beep(frequency, duration)
    # #     time_sound += 1 / FPS
    # #     time.sleep()


    # for s in range(1, total_distance_durch_73469):
    #     s *= 7.3469 
        
    # #     print(v_0 ** 2 - 4 * (a / 2) * (-s - 7.3469))

    #     t_1 = (-v_0 + math.sqrt(v_0 ** 2 - 4 * (a / 2) * (-s))) / a
    #     t_2 = (-v_0 + math.sqrt(v_0 ** 2 - 4 * (a / 2) * (-s - 7.3469))) / a
    ...
    #     try:
    #         time.sleep(1/60 - (t_2 - t_1) - duration)
    #     except:
    #         pass



def sound_while_rotation_second_try(number_before_rotation, number_after_rotation):
    global frequency
    
    time_sound = 1 / FPS
    number_of_rotations = 7
    total_time_sound = 7.3469 * ( number_after_rotation - number_before_rotation + 49 * number_of_rotations) / (0.5 * 7.3469 * FPS)
    
    while time_sound < total_time_sound:
        winsound.Beep(frequency, duration)
        frequency += 10
...
def draw_correct_numbers(number_of_wheel_rotations):
    ...
    # eval(f"correct_number_font_{number_of_wheel_rotations}") = pygame.font.Font(None,50)
    # eval(f"correct_number_rect_{number_of_wheel_rotations}") = pygame.Rect(606, 150 + 50 * number_of_wheel_rotations, 10, 30)
    # eval(f"correct_number_text_{number_of_wheel_rotations}") = (str(eval(f"correct_numbers[{int(number_of_wheel_rotations) - 1}]")))

    # if number_of_wheel_rotations == 0:
    #     eval(f"correct_number_text_{number_of_wheel_rotations}") = "Gewinnerzahlen:"
    #     eval(f"correct_number_font_{number_of_wheel_rotations}") = pygame.font.Font(None, 32)


    # for g in guess:
    #     if int(g) == eval("correct_numbers[" + str(number_of_wheel_rotations) + "]"):
    #         eval("color_of_correct_number_" + number_of_wheel_rotations) = GREEN

    # eval(f"correct_number_{number_of_wheel_rotations}_surface") = exec(f"correct_number_font_{number_of_wheel_rotations}.render(eval(f'correct_number_text_{number_of_wheel_rotations}, True, color_of_correct_number_{number_of_wheel_rotations}'))")
    # WIN.blit(eval(f"correct_number_{number_of_wheel_rotations}_surface"), (eval(f"correct_number_rect_{number_of_wheel_rotations}.x") + 5, eval(f"correct_number_rect_{number_of_wheel_rotations}.y") + 5))
...
def draw_window():
    ...
    if status_rotate_wheel and number_of_correct_number < 6:
        ...
        # process_1 = threading.Thread(target=rotate_wheel, args= (current_number, eval("correct_numbers[" + str(number_of_correct_number) + "]")))
        process_sound = threading.Thread(target=sound_while_rotation_second_try, args = (current_number, eval("correct_numbers[" + str(number_of_correct_number) + "]")))
